...
parse_primary_expression()
{
    ...
    my_get_token(&token);
//    printf("parse_primary_expression 1 token = %d\n", token.kind);
    ...
    my_get_token(&token);
//    printf("parse_primary_expression 2 token = %d\n", token.kind);
    if (token.kind == NUMBER_TOKEN) {
        value = token.value;
    } else if(token.kind == LEFT_PAREN_TOKEN) {
        value = parse_expression();
//        printf("parse_primary_expression 3 token\n");
        my_get_token(&token);
    ...
    return value * minus_flag; /* make compiler happy */
}
...
//    printf("parse_expression \n");
    ...
    for (;;) {
        ...
        v2 = parse_term();
//        printf("parse_expression v1 = %lf; v2 = %lf\n", v1, v2);
        if (token.kind == ADD_OPERATOR_TOKEN) {
            v1 += v2;
