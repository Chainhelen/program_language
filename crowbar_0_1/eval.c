        ...
        CRB_Value *result, int line_number)
{
    ...
    switch(operator){
    case BOOLEAN_EXPERSSION:
    case INT_EXPERSSION:
    case DOUBLE_EXPRESSION:
    case STRING_EXPERSSION:
    ...
    case MOD_EXPRESSION:
        result->u.int_value = left % right;
        break;
    case LOGICAL_AND_EXPERSSION:
    case LOGICAL_OR_EXPRESISON:
        DBG_panic(("bad case..%d", operator));
        break;
    case EQ_EXPRESSION:
...
static LocalEnviroment *
alloc_local_enviroment()
{
    LocalEnviroment *ret;

    ret = MEM_malloc(sizeof(LocalEnviroment));
    ret->variable = NULL;
...
static void 
dispose_local_environment(CRB_Interpreter *inter,LocalEnviroment *env)
{
    while(env->variable){
        Variable    *temp;
        temp = env->variable;
...
CRB_Value
crb_eval_expression(CRB_Interpreter *inter, LocalEnviroment *env,
        Expression *expr)
{
